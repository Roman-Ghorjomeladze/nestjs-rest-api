...
describe('AuthService', () => {
	...
	const signInTest = async (bcryptResult: boolean, findByEmailResult = UserMock) => {
		...
		jest.spyOn(bcrypt, 'compare').mockImplementation(jest.fn().mockResolvedValue(bcryptResult));
		const result = await service.signIn(SignInDtoMock);
		return result;
	}

	const failing = async () => {
		await signInTest(false);
	}
